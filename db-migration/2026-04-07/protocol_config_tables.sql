...
(
    ...
    `protocol_name`       VARCHAR(100) NOT NULL COMMENT '协议类型名称',
    `protocol_identifier` VARCHAR(50)  NOT NULL COMMENT '协议标识符',
    `applicable_system`   VARCHAR(50)  NOT NULL COMMENT '适用系统类型（CAD、ERP、PLM等）',
...
(
    ...
    `protocol_id`     BIGINT      NOT NULL COMMENT '协议类型ID',
    `parameter_name`  VARCHAR(50) NOT NULL COMMENT '参数名称（URL、PORT、AUTH_TYPE、TIMEOUT_CONNECT、TIMEOUT_READ、RETRY_COUNT、RETRY_INTERVAL、DATA_FORMAT等）',
    `parameter_value` VARCHAR(500) COMMENT '参数值',
    `is_sensitive`    TINYINT     NOT NULL DEFAULT 0 COMMENT '是否敏感字段：0-否，1-是',
...
(
    ...
    `parameters_json`      JSON         NOT NULL COMMENT '参数快照（JSON格式，包含URL、端口、认证方式等）',
    `create_by`           BIGINT       NOT NULL COMMENT '创建人',
    `create_time`           DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_by`           BIGINT COMMENT '更新人',
    `update_time`           DATETIME              DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
(
    ...
    `protocol_id`         BIGINT      NOT NULL COMMENT '协议类型ID',
    `test_type`           VARCHAR(20) NOT NULL COMMENT '测试类型：CONNECT-连接测试，DATA_TRANSFER-数据传输测试',
    `test_status`         TINYINT     NOT NULL COMMENT '测试结果：0-失败，1-成功',
    `response_time`       INT COMMENT '响应时间（毫秒）',
    `error_code`          VARCHAR(20) COMMENT '错误码（如401、500）',
    `error_message`       VARCHAR(500) COMMENT '错误信息',
    `test_data_sample_id` BIGINT COMMENT '测试数据样本ID',
    `create_by`          BIGINT      NOT NULL COMMENT '测试人',
...
(
    ...
    `sample_data`        TEXT         NOT NULL COMMENT '样本数据内容',
    `data_format`        VARCHAR(20)  NOT NULL COMMENT '数据格式：JSON、XML、CSV',
...
(
    ...
    `template_name`          VARCHAR(100) NOT NULL COMMENT '模板名称',
    `search_conditions_json` JSON         NOT NULL COMMENT '搜索条件JSON（包含名称、类型、状态、时间范围等）',
    `create_time`             DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`             DATETIME              DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
(
    ...
    `schema_version`    VARCHAR(20)  NOT NULL COMMENT 'Schema版本',
    `schema_definition` JSON         NOT NULL COMMENT 'Schema定义（JSON Schema格式）',
    `file_format`       VARCHAR(20)  NOT NULL COMMENT '文件格式：JSON、YAML、EXCEL',
...
(
    ...
    `file_format`       VARCHAR(20)  NOT NULL COMMENT '文件格式：JSON、YAML、EXCEL、CSV',
    `import_type`       VARCHAR(50)  NOT NULL COMMENT '导入类型：PROTOCOL_TYPE-协议类型，PROTOCOL_CONFIG-协议配置，PERMISSION-权限',
    ...
    `import_status`     TINYINT      NOT NULL COMMENT '导入状态：0-失败，1-成功，2-部分成功',
    `import_session_id` VARCHAR(100) COMMENT '导入会话ID',
    `import_start_time` DATETIME     NOT NULL COMMENT '导入开始时间',
...
(
    ...
    `file_size`         BIGINT COMMENT '文件大小（字节）',
    `file_format`       VARCHAR(20)  NOT NULL COMMENT '文件格式：JSON、YAML、EXCEL、CSV、PDF',
    `export_type`       VARCHAR(50)  NOT NULL COMMENT '导出类型：PROTOCOL_TYPE-协议类型，PROTOCOL_CONFIG-协议配置，TEST_RESULT-测试结果，PERMISSION-权限',
...
(
    ...
    `error_type`       VARCHAR(50)  NOT NULL COMMENT '错误类型：FORMAT_ERROR-格式错误，DUPLICATE_ERROR-重复错误，VALIDATION_ERROR-校验错误',
...
(
    ...
    `validation_type`    VARCHAR(50) NOT NULL COMMENT '校验类型：SCHEMA_VALIDATION-Schema校验，STRUCTURE_VALIDATION-结构校验',
    `validation_status`  TINYINT     NOT NULL COMMENT '校验状态：0-失败，1-成功',
...
(
    ...
    `file_type`    VARCHAR(50)  NOT NULL COMMENT '文件类型：IMPORT_FILE-导入文件，EXPORT_FILE-导出文件，REPORT-报告',
    `storage_type` VARCHAR(20)  NOT NULL COMMENT '存储类型：LOCAL-本地，OSS-对象存储',
...
(
    ...
    `import_session_id` VARCHAR(100) NOT NULL COMMENT '导入会话ID',
    `parameter_name`    VARCHAR(50)  NOT NULL COMMENT '参数名称',
    `parameter_value`   VARCHAR(500) COMMENT '参数值',
    `parameter_type`    VARCHAR(20)  NOT NULL COMMENT '参数类型：GLOBAL-全局参数，OVERRIDE-覆盖参数',
    `create_time`        DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
...
(
    ...
    `report_type`     VARCHAR(50)  NOT NULL COMMENT '报告类型：TEST_RESULT-测试结果报告，PROTOCOL_SUMMARY-协议汇总报告',
...
(
    ...
    `protocol_id`       BIGINT      NOT NULL COMMENT '协议类型ID',
    `permission_bitmap` VARCHAR(20) NOT NULL COMMENT '权限位图：C-创建，R-读取，U-更新，D-删除（如"CRUD"）',
    `create_by`        BIGINT      NOT NULL COMMENT '创建人',
    `create_time`        DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_by`        BIGINT COMMENT '更新人',
    `update_time`        DATETIME             DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
