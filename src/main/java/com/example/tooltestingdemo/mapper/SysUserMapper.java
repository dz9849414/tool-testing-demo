...
public interface SysUserMapper extends BaseMapper<SysUser> {
     ...
     * 根据用户名和状态查找用户
     */
    @Select("SELECT * FROM sys_user WHERE username = #{username} AND status = #{status} AND is_deleted = 0")
    SysUser selectByUsernameAndStatus(@Param("username") String username, @Param("status") Integer status);
    
    /**
     * 根据角色ID查找用户列表
     */
    @Select("SELECT u.* FROM sys_user u JOIN sys_user_role ur ON u.id = ur.user_id WHERE ur.role_id = #{roleId} AND u.is_deleted = 0")
