...
public class UserPermissionController {
    ...
    public Result<BatchRemoveResult> batchRemoveUserPermissions(@RequestBody BatchRemoveUserPermissionDTO dto) {
        try {
            // 参数校验
            ...
            BatchRemoveResult result = userPermissionService.batchRemoveUserPermissions(dto);
            
            if (result.getSuccess()) {
                return Result.success("批量移除用户直接权限成功");
            } else {
                // 构建详细的错误信息
                StringBuilder errorMessage = new StringBuilder("批量移除用户直接权限失败\n");
            ...
            errorResult.setMessage("批量移除失败: " + e.getMessage());
            
            return Result.error("批量移除用户直接权限失败：" + e.getMessage()+errorResult);
        }
    }
